...
struct WeatherData {
  ...
  String days[DAY_COUNT];
  String dates[DAY_COUNT];
  float tempsHigh[DAY_COUNT];
  float tempsLow[DAY_COUNT];
  uint8_t wmoCodes[DAY_COUNT];
  int pop[DAY_COUNT];          // Precipitation Probability (%)
  float precipSum[DAY_COUNT];  // Total Precip (in)
  float rainSum[DAY_COUNT];    // Rain (in)
  float snowSum[DAY_COUNT];    // Snow (in)
  float windMax[DAY_COUNT];    // Max Wind (mph)
  float gustsMax[DAY_COUNT];   // Max Gusts (mph)
  float cloudCover[DAY_COUNT];

  // Expanded Daily Array Fields
  String dailySunrise[DAY_COUNT];
  String dailySunset[DAY_COUNT];
  int dailyHumidity[DAY_COUNT];
...
bool sendCurrentRequest() {
  ...
  JsonDocument params;
  params["latitude"] = g_weather_latitude;
  params["longitude"] = g_weather_longitude;
  ...
  params["timezone"] = g_weather_time_zone;
...
bool sendDailyRequest() {
  ...
  params["latitude"] = g_weather_latitude;
  params["longitude"] = g_weather_longitude;
  params["forecast_days"] = DAY_COUNT;
...
void printMapCityData() {
  ...
  g_weather_latitude = doc["results"][0]["latitude"];
  g_weather_longitude = doc["results"][0]["longitude"];
  g_weather_time_zone = String(doc["results"][0]["timezone"]);
...
void printCurrentData() {
  ...
  weather.currentTemp = doc["current"]["temperature_2m"].as<double>();
  weather.pressure = doc["current"]["surface_pressure"].as<double>();
  weather.windSpeed = doc["current"]["wind_speed_10m"].as<double>();
  weather.windDirection = doc["current"]["wind_direction_10m"].as<int>();
  weather.rain = doc["current"]["rain_sum"].as<double>();
  weather.snow = doc["current"]["snowfall_sum"].as<double>();
  weather.precip = doc["current"]["precipitation_sum"].as<double>();
}
  ...
  if (hourlyTime.size() > 0) {
    ...
    for (size_t i = 0; i < hoursToPrint; i++) {
      ...
      Serial_printf("   %-8s  %4.1f °F  %3d%%       %5.2f in      %5.2f in      %5.2f in      %4.1f    %4.1f mph\n",
                    timeOnly,
                    hourlyTemp[i].as<double>(),
                    hourlyProb[i].as<int>(),
                    hourlyPrecip[i].as<double>(),
                    hourlyRain[i].as<double>(),
                    hourlySnow[i].as<double>(),
                    hourlyPress[i].as<double>(),
                    hourlyWind[i].as<double>());
    }
  }
}
...
void printDailyData() {
  ...
  size_t count = min((size_t)DAY_COUNT, dailyTime.size());
  ...
  if (count > 0) {
    ...
    for (size_t i = 0; i < count; i++) {
      ...
      Serial_printf("   %-8s  %4.1f / %4.1f deg F  %3d%%  %5.2f in %5.2f in %5.2f in %4.1f mph %4.1f mph  %s  %s   %3d%%  %s\n",
                    dateOnly,
                    dailyMax[i].as<double>(),
                    dailyMin[i].as<double>(),
                    dailyPop[i].as<int>(),
                    dailyPrecip[i].as<double>(),
                    dailyRain[i].as<double>(),
                    dailySnow[i].as<double>(),
                    dailyWind[i].as<double>(),
                    dailyGusts[i].as<double>(),
                    riseTime.c_str(),
                    setTime.c_str(),
                    humidity[i].as<int>(),
...
void printAirQualityData() {
  ...
  Serial_printf("PM 2.5:      %.1f hPa\n", doc["current"]["pm2_5"].as<double>());
  Serial_printf("PM 10:       %.1f hPa\n", doc["current"]["pm10"].as<double>());

  weather.AQI = doc["current"]["us_aqi"].as<int>();
  weather.PM25 = doc["current"]["pm2_5"].as<double>();
  weather.PM10 = doc["current"]["pm10"].as<double>();
}
